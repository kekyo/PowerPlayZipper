...
namespace PowerPlayZipper
{
    ...
    {
        public static void AssertCompareFiles(string basePath1, string basePath2)
        {
            var ppzFiles = new HashSet<string>(
                Directory.EnumerateFiles(basePath1, "*", SearchOption.AllDirectories).
                Select(ppzFile => ppzFile.Substring(basePath1.Length + 1)));
            var szlFiles = new HashSet<string>(
                Directory.EnumerateFiles(basePath2, "*", SearchOption.AllDirectories).
                Select(szlFile => szlFile.Substring(basePath2.Length + 1)));
            ...
            {
                using (var ppzStream = File.OpenRead(Path.Combine(basePath1, file)))
                {
                    using (var szlStream = File.OpenRead(Path.Combine(basePath2, file)))
                    {
